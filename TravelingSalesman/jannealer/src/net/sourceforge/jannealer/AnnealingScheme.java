...
public class AnnealingScheme {
	...
	/**
	 * Sets the Tolerance attribute of the AnnealingScheme object. The annealing
	 * stops once the return values from the ObjectiveFunction varies by less
	 * than the tolerance.
	 *
	 * @param tolerance
	 ...
	 *
	 * @param offset
	 *            The new Offset value. To reduce API tripwires, the offset is
	 *            duplicated, i.e. the caller can safely modify the passed in
	 *            offset it afterwards.
	 */
	 ...
	 **/
	public void anneal() {
		// doubt: what is this used for? comes from schedule, read schedule
		double ybb = 1.0e+30;
		double temptr = temperature;
		int iiter = nIterations;

		Schedule schedule = new Schedule(this);// doubt what does this class do?
		SimplexList simplexList = new SimplexList(ITOL);// doubt this too?
		Simplex simplex;// doubt and this?
		...
		for (;;) {
			...
			simplex = new Simplex(schedule.getPb(), schedule.getYb(),
					function.getNdim());
			if (simplexList.size() == ITOL) {
				simplexList.pruneMax();
			}
			simplexList.add(simplex);
			if (simplexList.size() == ITOL) {// doubt ye kyaa hua? understand
												// simplex for this
				if ((simplexList.spread() < tolerance)
						|| (loopLevel > getMaxIterations())) {
					setOffsetFromSimplex(simplexList.getMinSimplex()
							.getVertex());
					break;
				} else {
				}
	...
	private final static int ITOL = 10;// doubt ye kyaa? max size of simplex
										// list
	private final static long idum = -64;// doubt ye kyaa?
