...
public class LKH_algo_random {
	...
	public LKH_algo_random(List<double[]> coordinates) {
		...
		this.tour = new Tour(initTour(problemSize, 0), problemSize);
	}

	public void randomize() {
		this.tour = new Tour(initTour(problemSize,
				new Random().nextInt(problemSize)), problemSize);
	}

	private int[] initTour(int size, int startVertex) {
		int[] result = new int[size];
		// int startVertex = new Random().nextInt(size);
		for (int index = 0; index < size; index++) {
			int city = (index + startVertex) % size;
			result[index] = (city + 1) % size;
		}
		return result;
	}
