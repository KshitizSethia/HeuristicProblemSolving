...
public class GameState {
	...
	public static final int POS_INITIAL_WEIGHT = -4;
	public static final int WGT_INITIAL_WEIGHT = 3;
	...
	public GameState() {
		...
		board[POS_INITIAL_WEIGHT + HALF_BOARD] = new CocaineWeight(
				WGT_INITIAL_WEIGHT, POS_INITIAL_WEIGHT, PlayerName.none);
		count = 1;
	...
	private GameState(Stage stage, List<Integer> my_weights,
			List<Integer> oppn_weights, CocaineWeight[] board, int maxDepth,
			int count, PlayerName player) {
		this.stage = stage;
	 ...
	 * List<CocaineWeight> weights_on_board) { this.my_weights = weights;
	 * 
	 * oppn_weights = new LinkedList<Integer>(); for (int weight = 1; weight <=
	 * MAX_WEIGHT; weight++) { oppn_weights.add(weight); } board = new
	 * CocaineWeight[BOARD_SIZE]; for (CocaineWeight w : weights_on_board) {
	 * board[w.position] = w; if (w.player != player) { oppn_weights.remove(new
	 * Integer(w.weight)); // todo: see if this works } } this.stage = stage;
	 * 
	...
	public void makeMove(int weight, int position, PlayerName player) {
		board[position + HALF_BOARD] = new CocaineWeight(weight, position,
				player);
		count++;
	...
	public CocaineWeight removeMove(int i) {
		...
		} else if(removed.player!= PlayerName.none){
	...
	public boolean isTipping() {
		...
		for (int i = -HALF_BOARD; i <= HALF_BOARD; i++) {
			left_torque -= (i - (-3)) * getWeight(i);
			right_torque -= (i - (-1)) * getWeight(i);
		}
	...
	public int getAbsTorque() {
		return Math.abs(torque_1() - torque_2());
	}
	...
	public int getWeight(int i) {
		...
		/*
		 * if(stage==Stage.removal && player==PlayerName.Player2 &&
		 * board[i+HALF_BOARD].player != PlayerName.Player2){ //player2 when
		 * removing can remove only own pieces return 0; }
		 */
	...
	public PlayerName getPlayerAt(int position) {
		int pos = position + HALF_BOARD;
		if (board[pos] == null) {
